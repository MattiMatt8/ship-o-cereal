...
def checkout_address(request): # TODO: Make sure cart is not empty
    if request.method == "POST": # TODO: Go back to address page address is selected but can't continue
...
def checkout_card(request): # TODO: Make sure address has been selected & cart not empty
    cvc_error = None # TODO: Go back to address page address is selected but can't continue
    ...
    return render(request, "orders/checkout_card.html", {
        "checkout_card_id": order.card_id,
        "cvc_error": cvc_error
...
def checkout_confirm(request): # TODO: Make sure address and card have been selected & cart not empty
    ...
    return render(request, "orders/checkout_confirm.html", { # TODO: Probably pass in card, address and user as well
        "order": order,
